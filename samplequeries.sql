...
create view section_students as (
	select u.idno as userid, u.realname as studentname, c.name as classname, s.code as seccode
	from sections s
...
create view section_tutors as (
	select u.idno as userid, u.realname as tutorname, c.name as classname, s.code as seccode
	from sections s
		join classes c where (s.class = c.cid)
...
create view section_teachers as (
	select u.idno as userid, u.realname as teachername, c.name as classname, s.code as seccode
	from sections s
		join classes c where (s.class = c.cid)
